...
#ifdef VHEAP
#include "myvmalloc.c" // <-- Virtual memory allocator
...
#else
#include "mymalloc.c" // <-- memory allocator
...
int test_stability(int numIterations, unsigned long *tot_alloc_time,
                   unsigned long *tot_free_time)
{
    int i, n, res = 0;
    char s[80];
    ADDRS addr1;
...
int main(int argc, char **argv)
{
    ...
    print_heap_status(tot_alloc_time, tot_free_time, mem_size, 1);
// Test 2
#ifndef VHEAP
    ...
    printf("[%s%i KB%s]\n", KBLU, test_maxSizeOfAlloc(4 * 1024 * 1024) >> 10,
           KRESET);
    return 0;
}